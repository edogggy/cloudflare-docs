...
Some environment variables configure the Sandbox SDK itself rather than being passed to sandboxes:

- `SANDBOX_TRANSPORT` - Choose transport protocol (`http` or `websocket`). See [Transport configuration](/sandbox/configuration/transport/)
- `SANDBOX_LOG_LEVEL` - Control SDK logging level. See [Sandbox options](/sandbox/configuration/sandbox-options/#logging)
- `SANDBOX_LOG_FORMAT` - Control SDK log format. See [Sandbox options](/sandbox/configuration/sandbox-options/#logging)
- `SANDBOX_INSTANCE_TIMEOUT_MS` - Override container provisioning timeout. See [Sandbox options](/sandbox/configuration/sandbox-options/#containertimeouts)
- `SANDBOX_PORT_TIMEOUT_MS` - Override container startup timeout. See [Sandbox options](/sandbox/configuration/sandbox-options/#containertimeouts)

These variables affect SDK behavior and are read when your Worker starts.

## Related resources

- [Transport configuration](/sandbox/configuration/transport/) - Choose HTTP or WebSocket transport
