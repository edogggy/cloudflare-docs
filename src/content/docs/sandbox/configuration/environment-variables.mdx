...
**Environment variable**: `SANDBOX_TRANSPORT`
**Values**: `http` (default), `websocket`

Control how the SDK communicates with containers. By default, the SDK uses HTTP requests for each operation. Set to `websocket` to multiplex all SDK calls over a single persistent WebSocket connection.

**Why use WebSocket transport:**

- **Avoid sub-request limits** - Workers and Durable Objects have limits on concurrent sub-requests. WebSocket transport uses a single persistent connection instead of individual HTTP requests for each SDK operation.
- **Better performance** - Reusing a single connection eliminates per-request connection overhead.
- **Transparent** - No code changes needed, the SDK handles everything internally.
...
await sandbox.writeFile("/app/data.txt", "content");
// All operations use the WebSocket connection automatically
```

**When to use WebSocket transport:**

- You are hitting sub-request limits in Workers or Durable Objects
- You perform many SDK operations per request (file operations, commands, etc.)
- You want to reduce connection overhead for improved performance

**When to use HTTP transport (default):**

- You perform only a few SDK operations per request
- You do not experience sub-request limit issues
- You prefer the simpler HTTP request model for debugging

:::note[Transparent implementation]
WebSocket transport requires no code changes. The SDK automatically establishes and manages the WebSocket connection when you set the environment variable. All SDK methods work identically regardless of transport mode.
:::
