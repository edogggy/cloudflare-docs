...
1. **Client SDK** validates parameters and sends request to Durable Object (via HTTP or WebSocket)
...
- **HTTP transport (default)** - Each SDK operation makes a separate HTTP request
- **WebSocket transport** - All SDK operations multiplex over a single persistent WebSocket connection

Set the `SANDBOX_TRANSPORT=websocket` environment variable to enable WebSocket mode. This is transparent to your code—all SDK methods work identically regardless of transport mode. WebSocket transport helps avoid sub-request limits in Workers and Durable Objects. See [Environment variables](/sandbox/configuration/environment-variables/) for details.
